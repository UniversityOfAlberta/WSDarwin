...
//import com.google.code.apis.rest.client.Util.InvalidUriException;

/* 
 * @author Thomas Steiner (http://tomayac.com/)
...
public class Uri {
	  ...
	  protected void resolve(String uriString) {
	    try {
...
//	    } catch (InvalidUriException e) {    
	  ...
	  public String[] getPathComponentsAfterBase(String resourceBase) {    
		    String[] pathComponents = this.getPathComponents();
		    ArrayList<String> newPathComponents = new ArrayList<String>();
		    if (pathComponents[0].length() == 0) {
		      for (int i = 1; i < pathComponents.length; i++) {
		        if (!resourceBase.contains("/"+pathComponents[i]+"/")) {
					newPathComponents.add(pathComponents[i]);
				}
		      }
		    }
		    else {
		    ...
		    return newPathComponents.toArray(new String[newPathComponents.size()]);
		  }
